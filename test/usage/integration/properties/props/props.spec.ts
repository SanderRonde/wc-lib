import { PropsElement, ComplexReceiverElement, EmptyProps, PrivProps, MergedProps, PropsElementWindow, ReflectProps, OverriddenProp } from "./elements/props-element";
...
context('Props', function() {
	this.slow(SLOW);
	beforeEach(() => {
		cy.visit(getPropertyFixture('props'));
		cy.wait(250);
	});
		...
		context('Complex Types', () => {
			...
			context('Attribute Setting', () => {
				it('can be set through a reference initially', () => {
					cy.get('props-element')
						.shadowFind('#ref')
						.then(([receiver]: JQuery<ComplexReceiverElement>) => {
							expect(receiver)
								.to.have.property('props')
			...
			context('Watching', () => {
				context('Properties', () => {
					...
					context('No Proxy', () => {
						beforeEach(() => {
							cy.visit(getPropertyFixture('props', 'no-proxy'), {
								onBeforeLoad(win) {
