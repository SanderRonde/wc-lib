...
function decodeI18NExpression<LF>(node: ts.CallExpression,
...
function mapToObj<V>(map: Map<string, V>): {
	[key: string]: V;
} {
	const obj: {
		[key: string]: V;
 ...
 */
export function inlineI18N<LF extends { [key: string]: any;}>(text: string, 
	getMessage: (langFile: LF[keyof LF], key: string, values: any[]) => string,
	langFile: LF): { [key: string]: string };
export function inlineI18N<LF extends { [key: string]: any;}>(text: string, 
	getMessage: (langFile: LF[keyof LF], key: string, values: any[]) => string,
	langFile: LF, lang: keyof LF): string;
export function inlineI18N<LF extends { [key: string]: any;}>(text: string, 
	getMessage: (langFile: LF[keyof LF], key: string, values: any[]) => string,
	langFile: LF, lang?: keyof LF): string|{ [key: string]: string } {
		...
		const results = new Map((lang ? [lang] : Object.getOwnPropertyNames(langFile)).map((lang) => {
			...
			ast.forEachChild(child => AST.find({
				...
				decodeExpr(node) {
					return decodeI18NExpression(node as any, getMessage, langFile, lang);
				},
				findExpr(node) {
