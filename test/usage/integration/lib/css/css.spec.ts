...
context('Typed CSS', () => {
    ...
    context('Single string', () => {
        ...
        it('returns ? if the key is not a string', () => {
            expect(
                (css<BasicClass>().id as any)[Symbol() as any].toString()
            ).to.be.equal('#?');
        });
    });
    context('And', () => {
    ...
    context('Mixed And and Or', () => {
        it('can join more than two strings with and', () => {
            ...
            const expr = css<MultiClassStr>().id.a.and.b.and.c.and.d.and.e.and.f
                .and.g;
            expect(expr.toString()).to.be.equal('#a.b.c.d.e.f.g');
        });
        it('can join more than two strings with or', () => {
            const expr = css<BasicClass>().id.a.or.id.b.or.class.c.or.class.d.or
                .tag.e.or.tag.f;
            expect(expr.toString()).to.be.equal('#a, #b, .c, .d, e, f');
        });
        it('creates groups correctly when using both and and or', () => {
            const expr = css<BasicClass>().id.a.and.c.or.id.b.and.c.and.d.or.tag
                .e.or.tag.f;
        ...
        it('can chain on the inner class instance', () => {
            const expr = css<BasicClass>().id.a.and.c.orFn(
                css<BasicClass>().id.b.and.c.and.d.orFn(css<BasicClass>().tag.e)
            );
            expect(expr.toString()).to.be.equal('#a.c, #b.c.d, e');
        });
        it('can chain on the outer class instance', () => {
            const expr = css<BasicClass>()
                .id.a.and.c.orFn(css<BasicClass>().id.b.and.c.and.d)
                .orFn(css<BasicClass>().tag.e);
            expect(expr.toString()).to.be.equal('#a.c, #b.c.d, e');
        });
    });
    context('Toggleable classes', () => {
        ...
        it('can add multiple toggles', () => {
            const expr = css<BasicClass>().id.a.toggle['a-toggle'].toggle[
                'a-toggle-2'
            ];
    ...
    context('Pseudo selectors', () => {
        ...
        it('adds multiple pseudo selectors', () => {
            const expr = css<BasicClass>().id.a.pseudo(
                'hover',
                'active',
                'visited'
            );
    ...
    context('Children', () => {
        ...
        it('can add multiple children', () => {
            const expr = css<BasicClass>().id.a.child.$.b.child.class.c.child
                .class.d;
        ...
        it('can mix children with or selectors', () => {
            const expr = css<BasicClass>().id.a.child.$.b.or.class.c.child.class
                .d;
            expect(expr.toString()).to.be.equal('#a > #b, .c > .d');
        });
    });
    context('Descendants', () => {
        ...
        it('can add multiple descendants', () => {
            const expr = css<BasicClass>().id.a.descendant.$.b.descendant.class
                .c.descendant.class.d;
            expect(expr.toString()).to.be.equal('#a #b .c .d');
        });
        it('can mix descendants with and selectors', () => {
            const expr = css<BasicClass>().id.a.descendant.$.b.and.c.descendant
                .class.d;
            expect(expr.toString()).to.be.equal('#a #b.c .d');
        });
        it('can mix descendants with or selectors', () => {
            const expr = css<BasicClass>().id.a.descendant.$.b.or.class.c
                .descendant.class.d;
            expect(expr.toString()).to.be.equal('#a #b, .c .d');
        });
    });
    context('Children, Or and Descendant chains', () => {
        it('can chain children and descendants together', () => {
            {
                const expr = css<BasicClass>().id.a.child.$.b.descendant.class
                    .c;
                expect(expr.toString()).to.be.equal('#a > #b .c');
            }
            {
                const expr = css<BasicClass>().id.a.descendant.$.b.child.class
                    .c;
                expect(expr.toString()).to.be.equal('#a #b > .c');
            }
        });
        it('can join and, children and descendants together', () => {
            {
                const expr = css<BasicClass>().id.a.child.$.b.and.d.descendant
                    .class.c;
                expect(expr.toString()).to.be.equal('#a > #b.d .c');
            }
            {
                const expr = css<BasicClass>().id.a.descendant.$.b.and.d.child
                    .class.c;
                expect(expr.toString()).to.be.equal('#a #b.d > .c');
            }
        });
        it('can join or, children and descendants together', () => {
            {
                const expr = css<BasicClass>().id.a.child.$.b.or.$.a.descendant
                    .class.c;
                expect(expr.toString()).to.be.equal('#a > #b, #a .c');
            }
            {
                const expr = css<BasicClass>().id.a.descendant.$.b.or.$.a.child
                    .class.c;
                expect(expr.toString()).to.be.equal('#a #b, #a > .c');
            }
        });
