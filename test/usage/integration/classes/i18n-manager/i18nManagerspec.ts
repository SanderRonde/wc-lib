...
export function i18nManagerSpec(fixtures: {
	standard: string;
	root: string;
	defaults: string;
				...
				it('can set the language by setting prop_lang on the root', () => {
					...
					cy.get('root-element')
						.then(([el]: JQuery<RootElement>) => {
							expect(el.getLang()).to.be.equal('test', 'uses the globally set language');
						});
					getRootChildren().then((children) => {
						for (const child of children) {
							expect(child.getLang()).to.be.equal('test', 'uses the globally set language');
						}
					});
				});
			...
			context('Switching', () => {
				...
				it('can change the language while still loading the new one', () => {
					cy.get('#lang').then(([el]: JQuery<LangElement>) => {
						...
						el.setLang('en');
						expect(el.getLang()).to.be.equal('en', 'uses the old language');
						cy.wait(1000);
						cy.get('#lang')
