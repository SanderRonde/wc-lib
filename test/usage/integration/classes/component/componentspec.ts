			...
			it('calls the listener with the correct arguments', () => {
				cy.get('#test').then(([el]: JQuery<TestElement>) => {
					const listener = cy.spy((key, newValue, oldValue) => {
						...
						expect(newValue).to.be.equal(2, 'value was changed to 2');
			...
			it('calls the listener multiple times if the value changes multiple times', () => {
				cy.get('#test').then(([el]: JQuery<TestElement>) => {
					const listener = cy.spy((key, newValue, oldValue) => {
						...
						expect(newValue).to.not.be.equal(oldValue, 'new and old are not the same');
						expect(key).to.be.equal('x', 'changed value is x');
						expect(newValue).to.be.above(1, 'value was changed to a higher value');
					});
					el.listenProp('propChange', listener as any);
					el.props.x = 2;
					el.props.x = 3;
				...
				cy.get('#test').then(([el]: JQuery<TestElement>) => {
					...
					for (const selector of selectors) {
						expect([...el.$$(selector)]).to.have
							.same.members([...el.root.querySelectorAll(selector)], 'selectors return the same values');
					}
				});
			});
			context('Proxy', () => {
				it('runs querySelector on the local root when calling #$', () => {
					cy.get('#test').then(([el]: JQuery<TestElement>) => {
						...
						for (const selector of selectors) {
							expect(el.$(selector) === el.root.querySelector(selector), 'selectors return the same value')
								.to.be.true;
						}
					});
				});
			...
			context('No-Proxy', () => {
				...
				it('no longer has access to window.Proxy', () => {
					cy.window().then((window) => {
						expect((window as any).Proxy, 'proxy does not exist anymore').to.be.undefined;
					});
				});
				it('returns element with given ID when accessed', () => {
					cy.get('#test').then(([el]: JQuery<TestElement>) => {
