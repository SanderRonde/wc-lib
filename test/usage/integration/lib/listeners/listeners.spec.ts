...
context('Listeners', function() {
    ...
    context('#listen', () => {
        ...
        it('returns a function that will remove the listener', () => {
            cy.window().then((window: ListenerWindow) => {
                cy.get('#test').then(([el]: JQuery<TestElement>) => {
                    const listener = cy.spy();
                    const remove = window.Listeners.listen(
                        el,
                        'divId',
                        'click',
                        listener as any
        ...
        it('also listens if passive event listeners are not supported', () => {
            cy.visit(getLibFixture('listeners'), {
                onBeforeLoad(win) {
                    const originalAddEventListener = (win as any).__proto__
                        .__proto__.__proto__.addEventListener;
                    (win as any).__proto__.__proto__.__proto__.addEventListener = function<
                        T
                    >(
                        this: T,
                        event: string,
    ...
    context('#isNewElement', () => {
        it('returns false if no element is passed', () => {
            cy.window().then((window: ListenerWindow) => {
                expect((window.Listeners.isNewElement as any)()).to.be.equal(
                    false
                );
            });
        });
        it('will return true on the first call for any element', () => {
            cy.window().then((window: ListenerWindow) => {
                cy.get('#test').then(([el]: JQuery<TestElement>) => {
                    expect(
                        window.Listeners.isNewElement(el.$.divId)
                    ).to.be.equal(true);
                });
            });
        });
        ...
        it('will return true if a different element is passed', () => {
            cy.window().then((window: ListenerWindow) => {
                cy.get('#test').then(([el]: JQuery<TestElement>) => {
                    ...
                    expect(
                        window.Listeners.isNewElement(el.$.headerId)
                    ).to.be.equal(true);
                });
            });
        });
        it('can use a custom context', () => {
            cy.window().then((window: ListenerWindow) => {
                cy.get('#test').then(([el]: JQuery<TestElement>) => {
                    const context = {};
                    expect(
                        window.Listeners.isNewElement(el.$.divId, context)
                    ).to.be.equal(true);
                });
            });
        });
        ...
        it('will return true if the element is different but the context is the same', () => {
            cy.window().then((window: ListenerWindow) => {
                cy.get('#test').then(([el]: JQuery<TestElement>) => {
                    ...
                    expect(
                        window.Listeners.isNewElement(el.$.headerId, context)
                    ).to.be.equal(true);
                });
            });
        });
        ...
        it('throws an error if a non-object and non-function is used as the context', () => {
            cy.window().then((window: ListenerWindow) => {
                cy.get('#test').then(([el]: JQuery<TestElement>) => {
                    expect(() => {
                        expect(
                            window.Listeners.isNewElement(el.$.divId, 'ctx')
                        ).to.be.equal(true);
    ...
    context('#listenIfNew', () => {
        ...
        it('returns a function that will remove the listener', () => {
            cy.window().then((window: ListenerWindow) => {
                cy.get('#test').then(([el]: JQuery<TestElement>) => {
                    const listener = cy.spy();
                    const remove = window.Listeners.listenIfNew(
                        el,
                        'divId',
                        'click',
                        listener as any
    ...
    context('#listenToComponentUnique', () => {
        ...
        it('does allow the same listener for different events', () => {
            cy.window().then((window: ListenerWindow) => {
                let calls: number = 0;
                const original = (window as any).__proto__.__proto__.__proto__
                    .addEventListener;
                (window as any).__proto__.__proto__.__proto__.addEventListener = function(
                    event: string,
                    fn: Function,
                    options?: any
                ) {
                        ...
                        .then(() => {
                            ...
                            cy.get('#test')
                                ...
                                .then(() => {
                                    ...
                                    window.Listeners.removeAllElementListeners(
                                        el
                                    );
        ...
        it('removes all listeners added through multiple functions', () => {
            cy.window().then((window: ListenerWindow) => {
                cy.get('#test').then(([el]: JQuery<TestElement>) => {
                    ...
                    const listenComponentUnique = cy
                        .spy()
                        .as('componentUnique');
                    ...
                    window.Listeners.listen(
                        ...
                        'divId',
                        'click',
                        listenListener as any
                    );
                    window.Listeners.listenToComponentUnique(
                        el,
                        'click',
                        listenComponentUnique as any
                    );
                    window.Listeners.listenToComponent(
                        el,
                        'click',
                        listenComponent as any
                    );
                    ...
                    cy.get('#test')
                        ...
                        .then(() => {
                            ...
                            cy.wrap(listenComponentUnique).should(
                                'be.calledOnce'
                            );
                            ...
                            cy.get('#test')
                                ...
                                .shadowClick()
                                .then(() => {
                                    cy.wrap(listenListener).should(
                                        'be.calledTwice'
                                    );
                                    cy.wrap(listenComponentUnique).should(
                                        'be.calledTwice'
                                    );
                                    cy.wrap(listenComponent).should(
                                        'be.calledTwice'
                                    );
