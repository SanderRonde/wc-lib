import {
    ...
    ReflectProps,
    OverriddenProp,
} from './elements/props-element';
import { SLOW } from '../../../lib/timing';

export function propsSpec(
    basicFixture: string,
                        ...
                        it('can be disabled for bools', () => {
                            cy.get('props-element').then(
                                ([el]: JQuery<PropsElement>) => {
                                    ...
                                    expect(el).to.not.have.property(
                                        'noReflectBool'
                                    );
                                }
                            );
                    ...
                    context('Reflecting to self', () => {
                        context('Enabled', () => {
                            ...
                            it('does not override already defined properties if they are defined before props', () => {
                                cy.get('overridden-component').then(
                                    ([el]: JQuery<OverriddenProp>) => {
                                        expect(el)
                                            .to.have.property('props')
                                            .to.have.property('bool')
                                            .to.be.a('boolean')
                        ...
                        cy.window().then((win: PropsElementWindow) => {
                            ...
                            const stub = cy.stub(
                                ...
                                (props) => {
                                    if (calls) return;
                                    calls++;
                                    expect(props.noVal).to.have.property(
                                        'a',
                                        'b'
                                    );
                                }
                        ...
                        it('does not overwrite passed ref', () => {
                            cy.window().then((win: PropsElementWindow) => {
                                ...
                                const stub = cy.stub(
                                    ...
                                    (props) => {
                                        if (calls) return;
                                        expect(props.ref).to.have.property(
                                            'e',
                                            'f'
                                        );
                                        calls++;
                    ...
                    it('does not overwrite passed non-ref', () => {
                        cy.window().then((win: PropsElementWindow) => {
                            ...
                            const stub = cy.stub(
                                ...
                                (props) => {
                                    if (calls) return;
                                    expect(props.noref).to.have.property(
                                        'g',
                                        'h'
                                    );
                                    calls++;
                    ...
                    context('Nested', () => {
                        ...
                        it('can watch simple array element changes', () => {
                            cy.get('props-element').then(
                                ([el]: JQuery<PropsElement>) => {
                                    const stub = cy.stub(el, 'renderToDOM');
                                    el.props.watchNestedArrProp[0].a = {
                                        a: 'd',
                                        c: 'b',
                                    };
                                    cy.wrap(stub)
        ...
        context('Defining', () => {
            context('Config', () => {
                ...
                it('can consist of a private config', () => {
                    cy.get('priv-props').then(([el]: JQuery<PrivProps>) => {
                        expect(el)
                            .to.have.property('props')
                            .to.have.property('bool');
                        expect(el)
                ...
                it('can consist of a public config', () => {
                    cy.get('reflect-props').then(([el]: JQuery<PrivProps>) => {
                        expect(el)
                            .to.have.property('props')
                            .to.have.property('bool');
                        expect(el)
            ...
            context('Mixin', () => {
                it('can inherit props from a mixin', () => {
                    cy.get('merged-props').then(([el]: JQuery<MergedProps>) => {
                        expect(el)
                            .to.have.property('props')
                            .to.have.property('bool');
                        expect(el)
                    ...
                    cy.document().then((document) => {
                        cy.on('uncaught:exception', (err) => {
                            expect(err.message).to.include(
                                'Parent props should be a Props object'
                            );
                            done();
                            return false;
                        });
