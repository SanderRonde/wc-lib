...
class CSS {
    ...
    callFn(args: ts.NodeArray<ts.Expression>) {
        ...
        this._try(() => {
            this.instance = this.instance.call(
                this._lastInstance,
                ...basicArgs
            );
        });
...
function decodeCSSExpression(
    node:
        ...
        | ts.ElementAccessExpression,
    noStr: false
): { str: string; lastNode: ts.Node } | null;
function decodeCSSExpression(
    node:
        | ts.CallExpression
        | ts.PropertyAccessExpression
...
function decodeCSSExpression(
    node:
        ...
        | ts.ElementAccessExpression,
    noStr: boolean = false
): { str: string; lastNode: ts.Node } | null | { toString(): any } {
    // istanbul ignore next
    ...
    const css = new CSS();
    while (
        (ts.isCallExpression(activeNode.parent) ||
        ...
        if (ts.isCallExpression(activeNode)) {
            ...
            const stringLiteral = AST.resolveStringLiteral(
                activeNode.argumentExpression
            );
