...
abstract class AllCSSMap<S extends SelectorMap> {
    constructor(
        private _onValue?: (
            sel: CSSSelector<S, any, any, any>
        ) => CSSSelector<S, any, any, any>
    ) {}
...
>(
    prefix: string,
    preRet?: (sel: CSSSelector<S, T, M, any>) => CSSSelector<S, T, any, any>
): {
    [K in keyof M]: CSSSelector<S, T, M, K>;
} {
    return new Proxy(
...
> {
    ...
    and = genProxy<S, 'CLASSES', DefaultObj<S['CLASSES']>>('.', (sel) => {
        ...
        [K in keyof DefaultObj<S['CLASSES']>]: CSSSelector<
            S,
            'CLASSES',
            DefaultObj<S['CLASSES']>,
            K
        >;
    };

    private _registerDiffSelector(
        container: DiffSelector<S>,
        selector: CSSSelector<S, any, any, any>
    ) {
        ...
        parent[container.name].group = parent[container.name].group.filter(
            (v, i, a) => a.indexOf(v) === i
        );
    ...
    private _toString(
        ignore: WeakSet<CSSSelector<S, any, any, any>> = new WeakSet()
    ): string | null {
        ...
        const {
            pre: descendantPre,
            post: descendantPost,
        } = this._collapseDiffSelector(this._descendantSelectors, ignore);
...
export function css<C>(_c?: C): CSS<InferSelectors<C>> {
    try {
        ...
        throw new Error(
            'Attempting to use css map while proxy is not supported'
        );
    }
}
