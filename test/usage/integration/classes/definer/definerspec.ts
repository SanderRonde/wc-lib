        ...
        context('Defining', () => {
            ...
            it('defines the component when calling #define', () => {
                cy.window().then((window: TestParentWindow) => {
                    expect(window.customElements.get('parent-element')).to.be
                        .undefined;
                    window.ParentElement.define(true);
                    expect(
                        window.customElements.get('parent-element')
            ...
            it('defines any dependencies when calling #define', () => {
                cy.window().then((window: TestParentWindow & TestWindow) => {
                    expect(window.customElements.get('parent-element')).to.be
                        .undefined;
                    expect(window.customElements.get('test-element')).to.be
                        .undefined;
                    window.ParentElement.define(true);
                    expect(
                        window.customElements.get('parent-element')
                    ...
                    expect(
                        window.customElements.get('test-element')
        ...
        context('Metadata', () => {
            ...
            it('has defined 0 elements initially', () => {
                cy.window().then((window: DefineMetaDataWindow) => {
                    expect(window.DefineMetadata)
                        .to.have.property('defined')
                        .to.be.equal(0, 'no elements are defined initially');
                });
            });
            it('increases the counter when a component is defined', () => {
                cy.window().then(
                    (window: DefineMetaDataWindow & TestParentWindow) => {
                        expect(window.DefineMetadata)
                            .to.have.property('defined')
                            .to.be.equal(
                                0,
                                'no elements are defined initially'
                            );
                        window.ParentElement.define(true);
                        expect(window.DefineMetadata)
                            .to.have.property('defined')
