...
export function listenerSpec(fixture: string) {
		...
		context('Firing', () => {
			...
			it('returns the return values of the listeners', () => {
				cy.get('#test').then(([el]: JQuery<TestElement>) => {
					...
					expect(retVals).to.have.length(expectedValues.length, 'retvals is the same length as the amount of listeners')
						.to.be.deep.equal(expectedValues, 'return values match');
				});
			});
		});
	});
