import {
    inlineI18N,
    inlineI18NPipe,
} from '../../../../../build/es/tasks/inline-i18n.js';
...
context('Inline I18N Task', () => {
    ...
    const defaultReplaceArgs = [
        ...
        (langFile: any, key: string, values: any[]) => string,
        {
            [key: string]: any;
        },
        string
    ...
    context('#inlineI18NPipe', () => {
        it('replaces I18N when file is a string', (done) => {
            ...
            newStream.on('end', () => {
                expect(result).to.be.equal('html`<div>ae</div>`');
                done();
            });
        });
    });
    context('#inlineI18N', () => {
        ...
        it('replaces single string access when using this.__', () => {
            ...
            }
            {
                const str = 'this.__("a")';
                expect(
                    inlineI18N(genStr(str), ...defaultReplaceArgs)
                ).to.be.equal(genExpected('ae'));
            }
            {
        ...
        it('replaces all languages if no language is passed', () => {
            const str = 'this.__("a")';
            const result = inlineI18N(
                str,
                defaultReplaceArgs[0],
                defaultReplaceArgs[1]
            );
            for (const lang in langFile) {
                expect(result).to.have.property(lang);
                const langResult = result[lang as keyof typeof lang];
                expect(langResult).to.be.equal(
                    `'${defaultReplaceArgs[0](
                        langFile[lang as keyof typeof langFile],
                        'a',
                        []
                    )}'`
                );
