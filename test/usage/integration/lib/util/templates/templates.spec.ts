    ...
    context('#joinTemplates', () => {
        it('can merge an empy array of templates', () => {
            cy.get('#test').then(([el]: JQuery<TestElement>) => {
                ...
                expect(
                    template
                        .renderAsText(CHANGE_TYPE.FORCE, el)
                        .replace(/\s/g, '')
                ).to.be.equal('');
            });
        });
        ...
        it('sets changeType to CHANGE_TYPE.PROP if passed two templates with that change type', () => {
            cy.get('#test').then(([el]: JQuery<TestElement>) => {
                ...
                expect(
                    template
                        .renderAsText(CHANGE_TYPE.PROP, el)
                        .replace(/\s/g, '')
                ).to.be.equal('a,b');
                expect(template)
                    .to.have.property('changeOn')
        ...
        it('sets changeType to CHANGE_TYPE.THEME if passed two templates with that change type', () => {
            cy.get('#test').then(([el]: JQuery<TestElement>) => {
                ...
                expect(
                    template
                        .renderAsText(CHANGE_TYPE.THEME, el)
                        .replace(/\s/g, '')
                ).to.be.equal('a,b');
                expect(template)
                    .to.have.property('changeOn')
        ...
        it('sets changeType to CHANGE_TYPE.LANG if passed two templates with that change type', () => {
            cy.get('#test').then(([el]: JQuery<TestElement>) => {
                ...
                expect(
                    template
                        .renderAsText(CHANGE_TYPE.LANG, el)
                        .replace(/\s/g, '')
                ).to.be.equal('a,b');
                expect(template)
                    .to.have.property('changeOn')
        ...
        it('sets changeType to CHANGE_TYPE.ALWAYS if passed at least one template with that change type', () => {
            cy.get('#test').then(([el]: JQuery<TestElement>) => {
                ...
                expect(
                    template
                        .renderAsText(CHANGE_TYPE.ALWAYS, el)
                        .replace(/\s/g, '')
                ).to.be.equal('a,b');
                expect(template)
                    .to.have.property('changeOn')
